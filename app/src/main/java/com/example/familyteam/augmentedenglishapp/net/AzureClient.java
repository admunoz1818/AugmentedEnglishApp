...
public abstract class AzureClient<T> implements TableOperationCallback<T>, TableDeleteCallback, TableQueryCallback<T> {
    ...
    public AzureClient(Context context){
        try {
            client = new MobileServiceClient("https://english-kids.azure-mobile.net/",
                    "vAkWjVMWGNJUIuercasIDXdEFYANUW62",
                    context);
    ...
    public void getAllItems(){ getTable().execute(this);}

    public void getLastItems(int size){ getTable().skip(size).execute(this);}
